...
final class DNSServerTests: XCTestCase {
    ...
    func testDNSResolution() async throws {
        ...
        let client = try await DNSClient.connect(
            on: group,
            config: [try SocketAddress(ipAddress: "127.0.0.1", port: port)]
        ).get()
        ...
        if let record = results.answers.first, case .a(let resourceRecord) = record {
            // 127.0.0.1 -> 2130706433
            XCTAssertEqual(resourceRecord.resource.address, 2130706433, "Address should be 127.0.0.1")
        } else {
