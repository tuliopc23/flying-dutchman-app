...
private final class PortForwardHandler: ChannelInboundHandler, @unchecked Sendable {
    ...
    func channelActive(context: ChannelHandlerContext) {
        ...
        Task {
            do {
                ...
                logger.error("Failed to establish VSOCK connection", metadata: ["error": .string(error.localizedDescription)])
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        // Use local copy of handle if possible, or access self.vsockHandle (data race risk acknowledged for stabilization)
