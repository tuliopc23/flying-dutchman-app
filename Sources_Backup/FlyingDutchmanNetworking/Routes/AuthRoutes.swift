...
struct AuthRoutes {
    ...
    private func login(_ request: Request, _ context: BasicRequestContext) async throws -> Response {
        ...
        guard let containerizationRuntime = runtime as? ContainerizationRuntime else {
            return errorResponse(message: "Authentication not supported by current runtime", status: .serviceUnavailable)
        }
        
        // Perform login
        do {
            try await containerizationRuntime.login(registry: payload.registry, username: payload.username, password: payload.password)
    ...
    private func encodeResponse<T: Encodable>(_ value: T, status: HTTPResponse.Status) throws -> Response {
