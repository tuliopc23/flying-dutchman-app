...
5) **CLI**: `swift run FlyingDutchmanCLI version`, `doctor`, `containers list`, `containers start <id|name>`, `containers stop <id|name>`; set `DOCKER_HOST=unix:///var/run/flyingdutchman-docker.sock` for shim tests.
