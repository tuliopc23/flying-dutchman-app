...
public final class MachineStore: Sendable {
    ...
    public func create(_ machine: Machine) throws {
        try dbQueue.write { db in
            try db.execute(
                sql: """
                INSERT INTO machines (id, name, distro, version, status, cpuCount, memoryGB, diskGB, ipAddress, sshPort, createdAt, updatedAt)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                arguments: [
                    machine.id,
    ...
    public func update(_ machine: Machine) throws {
        try dbQueue.write { db in
            try db.execute(
                sql: """
                UPDATE machines
                SET name = ?, distro = ?, version = ?, status = ?, cpuCount = ?, memoryGB = ?, diskGB = ?, ipAddress = ?, sshPort = ?, updatedAt = ?
                WHERE id = ?
                """,
                arguments: [
                    machine.name,
