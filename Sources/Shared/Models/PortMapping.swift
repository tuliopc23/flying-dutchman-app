...
public enum PortMappingError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
        switch self {
        case .invalidFormat(let spec, let reason):
            return "Invalid port mapping format '\(spec)': \(reason)"
        case .invalidProtocol(let proto):
            return "Invalid protocol '\(proto)'. Expected 'tcp' or 'udp'"
        case .invalidPort(let value, let field):
            return "Invalid \(field) '\(value)'. Expected a number between 1-65535"
        case .portOutOfRange(let port, let field):
            return "\(field) \(port) is out of range. Must be between 1-65535"
        }
    }
}
