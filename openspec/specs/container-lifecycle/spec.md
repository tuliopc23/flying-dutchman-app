...
The container-lifecycle capability provides the core functionality for managing container lifecycles using Apple's Containerization framework. This includes creating containers from OCI images, starting/stopping containers with resource limits, managing container state, and handling container lifecycle events robustly.

The runtime supports multiple backends (native Containerization framework, CLI fallback, stub for development) and ensures state synchronization between runtime and persistence layers.
